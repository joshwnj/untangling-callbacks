...
Then we simply count the number of non-empty lines in each file, record the maximum, and send back the corresponding filename. Seems like the obvious approach, right?
...
function findMaxFileInDirectory (directory, callback) {
  ...
  fs.readdir(directory, function (err, filenames) {
    ...
    // keep track of how many files need to be checked
...
The way we translate our requirements into functions can have a huge impact on the end result. There are a few techniques we'll look at for this example:
...
Before we start refactoring code it can be helpful to take a fresh look at the requirements, and see whether we can do any simplification there. To recap, our requirements are: `Given a directory, find the file...`. For our purposes we could say that a directory is equivalent to a list of files, right? We could reframe the problem as `Given a list of files, find the file containing the most non-empty lines.` So we can simplify by splitting the work into 2 separate functions:

```js
function findMaxFileInDirectory (directory, callback) {
  // get a list of filenames and pass into findMaxFile
  // ...
}
...
This might seem moot, but consider that in `findMaxFile` we've gone from dealing with _"directories and files"_ to just _"files"_. We have one less thing to consider, and the code will be cleaner as a result.

We can make a similar simplification when we look at the second half of the requirements: `find the file containing the most non-empty lines.` When you think about it we're not really concerned with counting lines in a file - we're counting lines of a string! That string just happens to be the contents of a file, but strings are simpler to work with than files. So we can easily factor out that logic without any reference to loading files:
...
This does the job, and it looks nice and familiar. But loops (eg. `for`) and conditions (eg. `if`) can quickly become very complex. Every time you see an `if` you must consider each possible outcome and trace the steps. Throw in a `break` or `continue` as well, and you'll need to account for that. And each logic branch compounds upon the complexity of the past.
...
We can apply the same technique when we load files. In the original example we were processing one file at a time, each time counting the lines and then recording that number if a new maximum was found:
...
It's a familiar pattern, but we can simplify things by first loading all files, then counting lines of all files, and finally discovering the maximum:
...
A common feature of the first two principles is that they help us write functions that have little or no knowledge of the context they are being used in. `findMaxFiles` doesn't know (or care) if the files came from a directory listing or somewhere else. `countNonEmptyLines` doesn't need to know that the string came from a file. `indexOfMax` only knows about arrays of numbers.

If a function is too tangled with its context you have to think much harder about using it. The less outside-knowledge a function has, the more confidently you can use it.
